...
export const Route = createFileRoute("/_authed/_scan/borrow-scan/")({
...
function BorrowScan({ videoRef }: ScannerProps) {
		...
		scannerRef.current = new QrScanner(
			...
			async (result) => {
				try {
					...
					scannerRef.current?.stop(); // Stop scanning once found
				} catch (error) {
					console.error(error);
					// Optional: Don't stop scanning on error, just log it or show toast
					// setScanError(true);
				}
			},
			{
	...
	if (scanError)
		return (
			<Failed
				...
				fn={resetState}
				header="Failed to scan borrow request."
			/>
		);
	if (mutation.isError)
		return (
			...
			{selectedRequest && (
				<DrawerContent className="space-y-4 h-full">
					<RequestDetailsHeader
						...
						date={selectedRequest.borrowedAt}
					/>
					<div className="px-4 py-4 overflow-y-auto space-y-4">
						...
						<DrawerFooter className="mt-0">
							<Button
								onClick={() =>
									mutation.mutate({
										id: selectedRequest.borrowRequestId,
	...
	return (
		<div>
			{equipments.map((equipment: any) => {
				// Handle ID difference between borrow and return items
				const key =
